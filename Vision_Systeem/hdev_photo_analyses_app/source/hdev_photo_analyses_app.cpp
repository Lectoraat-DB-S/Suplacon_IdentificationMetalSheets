...
#include "hdev_photo_analyses_app.h"
...
namespace hdev_photo_analyses_app {
  ...
  {     
    ...
    static HDevEngineCpp::HDevProcedure procedure(GetProgram(sgResourcePath+"Fotoanalyse_Applicatie.hdev"),"find_digits");
    HDevEngineCpp::HDevProcedureCall call=procedure;
    ParamHandler<HalconCpp::HObject>::SetParameter(call,"Image",Image);

		//HTuple *Test = ParamHandler<HalconCpp::HTuple>::GetParameter(call, "Area");
        
	call.Execute();

    if (Area != nullptr)
      * Area = ParamHandler<HalconCpp::HTuple>::GetParameter(call,"Area");
    if (Confidence != nullptr)
      * Confidence = ParamHandler<HalconCpp::HTuple>::GetParameter(call,"Confidence");
